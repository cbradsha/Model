...
%         dx_piston=1;
        ...
        p.P_brent(1) = 63.2932;   %lower power guess
        p.P_brent(2) = 8; %second lower power guess
        error_brents = 0.03;
        error_stroke = 1;
        ...
        p.dP_max=p.P_d-p.P_i;
            ...
            while (abs(dT_loop_k)>error || abs(drho_loop_k)>error) || (abs(dT_cv2_loop_k)>error || abs(drho_cv2_loop_k)>error) || abs(dx_piston)>error
                ...
                while size(t_cross)<p.n_period*2-1
                    ...
                    F_drive(i)=p.F_drive_max*(cos(p.w_d*(t(i)-p.t_force_adjust)+pi));                %Driving force v. Time, Newtons
            ...
            if m_dot(k-1) > 0
                ...
                T_d = sum(T(i_v2:i_v3))/(i_v3 - i_v2);
                h_2 = sum(h(i_v2:i_v3))/(i_v3 - i_v2);
                h_2_calc = EOS(T_d,p.P_d,'enthalpy','P');
                
                %Boundary work calculations
                W_2_3 = sum(P(i_v2:i_v3).*V(i_v2:i_v3))/(i_v3 - i_v2);
                W_4_1 = sum(P(i_v4:end).*V(i_v4:end))/(length(P) - i_v4);
                W_1_2 = sum(P(i_v1:i_v2).*V(i_v1:i_v2))/(i_v2 - i_v1);
                W_3_4 = sum(P(i_v3:i_v4).*V(i_v3:i_v4))/(i_v4 - i_v3);
                
                %Actual boundary work of real compression process
                W_boundary = W_1_2 + W_2_3 - W_3_4 - W_4_1;
                W_dot_boundary = W_boundary*f_list(l)*(1/pi);
                ...
                W_dot_net_ideal = W_gas_net_ideal*f_list(l)*(1/pi);
            ...
            W_dot(k-1)=m_dot(k-1)*(h_2-p.h_in); %W_dot is in kW
            ...
            W_dot_piston = F_drive_max*x_stroke_resonant_save(l)*f_list(l)*(4/pi);
            W_dot_stored = 0.5*p.k_mech*x_stroke_resonant_save(l)^2*f_list(l)*(2/pi);
            
            W_gas=((p.gamma*p.P_i*(max(V) - p.V_dead_valve))/(p.gamma-1))*(((p.P_d/p.P_i)^((p.gamma-1)/p.gamma))-1); %kJ
            W_dot_gas = W_gas*f_list(l)*(1/pi);
        ...
        if l>1
            ...
            change_in_power = P_electric_save(l) - P_electric_save(l-1);

            if change_in_power > 0
                number_change_power = number_change_power +1;
            end

            if number_change_power < 3                 

                p.resonant_loop = 0;
                f_resonant_calculated = 0;
                w_d_resonant_calculated = 0;

            elseif number_change_power >= 3      % is power increasing

                p.resonant_loop = 1;

                if length(P_electric_save) < 5

                    f_resonant_calculated = 99;
                    w_d_resonant_calculated = 99;

                elseif length(P_electric_save) <= 15 && length(P_electric_save) >=5

                    coefficients = polyfit(f_list_resonant_save,P_electric_save,2);            
                    f_resonant_calculated = -coefficients(2)/(2*coefficients(1));
                    w_d_resonant_calculated = f_resonant_calculated*2*pi;                

                else

                    P_electric_save = P_electric_save(end-14:end);
        ...
        if pc_flag == 0 %&& p.resonant_loop == 1
            ...
            range_2=strcat('A',num2str(l+rows),':','AQ',num2str(l+rows));
